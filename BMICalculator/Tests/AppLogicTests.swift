...
@Suite("ReminderCadence")
struct ReminderCadenceTests {

    typealias Cadence = NotificationService.ReminderCadence

    @Test("Four cadences, all raw-value round-trippable")
    func cases() {
        #expect(Cadence.allCases.count == 4)
        for c in Cadence.allCases {
            #expect(Cadence(rawValue: c.rawValue) == c)
            #expect(c.id == c.rawValue)
        }
    }

    @Test("Titles and subtitles are populated; Off reads as off")
    func copy() {
        #expect(Cadence.off.title == "Off")
        #expect(Cadence.weekly.title == "Weekly")
        for c in Cadence.allCases {
...
@Suite("ProfilePreferences")
struct ProfilePreferencesTests {

    @Test("Active id round-trips through the store and clears on nil")
    func roundTrip() {
        let suite = "test.profilePrefs"
        let defaults = UserDefaults(suiteName: suite)!
        defaults.removePersistentDomain(forName: suite)

        #expect(ProfilePreferences.activeID(defaults: defaults) == nil)

        let id = UUID()
        ProfilePreferences.setActiveID(id, defaults: defaults)
        #expect(ProfilePreferences.activeID(defaults: defaults) == id)

        ProfilePreferences.setActiveID(nil, defaults: defaults)
        #expect(ProfilePreferences.activeID(defaults: defaults) == nil)
    }
}
